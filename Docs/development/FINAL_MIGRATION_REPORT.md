...
- render: CH1〜CH4、PS1→PS2 50%、6puzzleの11 command captureを
  `apps/console-chaos/tests/fixtures/render-command-golden.json` に固定。draw順を含む全command hash一致。
...
- audio: 48 kHz stereo、4世代×14.4秒。peak/RMS/fingerprint一致、0.25秒無音窓0、peak 1未満。
  PCM hashは `apps/console-chaos/tests/fixtures/audio-pcm-golden.json` に固定。
