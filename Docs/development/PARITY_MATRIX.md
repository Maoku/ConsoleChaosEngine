...
`tools/fixtures/reference-snapshot.json` に固定し、最終確認でも HEAD一致・clean を確認した。
...
| production E2E | legacy page | Console本編でpuzzle、1→2→3→4、audio unlock、reset、dispose | pass |
...
- runtime renderer は legacy WebGL monolithをengine所有のgeneric WebGL passへ移した。Canvas fallbackは
  production bootstrapでは使わない。`apps/console-chaos/tests/fixtures/render-command-golden.json` は描画順を含む11 captureを固定する。
...
- `apps/console-chaos/tests/fixtures/audio-pcm-golden.json` はFC/SFC/PS1/PS2のpeak、RMS、先頭PCM fingerprintを固定する。
