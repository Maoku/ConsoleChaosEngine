...
- 52出力（logo 4、9 source state×4世代、PS2 body 3＋他世代の1×1 helper 9）の `assets:check`、共有FC 17色、RGB555、alpha mode、bounds、2回目buildのwritten 0を確認した
- PS2は顔領域を透明化した280×336 body 3枚と146×97の顔pattern 9枚へ変換した。顔領域＋10 pxの138×89 parity範囲が変更前の全身frame表示と3姿勢×3眼状態すべてでpixel完全一致し、外側4 pxの最外周が開眼bodyとRGBA一致することを自動検査した
- runtimeでPS2顔patternの拡縮、`tweenTexture`、`textureMix`、`source-over` hardware blendを使用しないことを自動検査した
