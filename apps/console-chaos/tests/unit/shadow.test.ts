...
const pillarEntity = area1.entities.find((entity) => entity.id === 'p2_1_pillar_a')!;
const pillarMaterial = materialFor(pillarEntity.type, pillarEntity.id);
const pillarCenter = pillarEntity.transform.position as [
  number,
  number,
  number,
];
const pillarHalf = pillarEntity.collider!.halfExtents;
const groundY = -4;
...
describe('松明（点光源）の落ち影', () => {
  it('柱は影を落とす（本体も描かれる。透明な塊はもう置かない）', () => {
    expect(pillarMaterial.castShadow).toBe(true);
    // 影が着地する床は柱が立っている足場（天面 -4）
    expect(groundY).toBeCloseTo(-4, 6);
  });

  it('影は光と反対側へ伸びる', () => {
    const quad = projectShadowQuad(pillarCenter, pillarHalf, groundY, torchAt(3));
  ...
  it('松明が回り込むと影も回り込む（動くことで柱の存在が分かる）', () => {
    const right = projectShadowQuad(pillarCenter, pillarHalf, groundY, torchAt(3));
    const left = projectShadowQuad(pillarCenter, pillarHalf, groundY, torchAt(-3));
  ...
  it('近づくほど影は大きく伸びる（平行光との違い）', () => {
    ...
    const stretched = projectShadowQuad(pillarCenter, pillarHalf, groundY, low);
    const compact = projectShadowQuad(pillarCenter, pillarHalf, groundY, high);
  ...
  it('光源が柱より低ければ影は落ちない', () => {
    ...
    expect(projectShadowQuad(pillarCenter, pillarHalf, groundY, below).strength).toBe(0);
  });

  it('影は床の天面のすぐ上に置かれる（床に埋まらない）', () => {
    const quad = projectShadowQuad(pillarCenter, pillarHalf, groundY, torchAt(3));
    expect(quad.center[1] - groundY).toBeCloseTo(SHADOW_LIFT, 6);
  });
  ...
  it('松明を持つのは動的ライトを持つ世代だけ（CH 4 のみ）', () => {
    const withTorch = (['FC', 'SFC', 'PS1', 'PS2'] as const).filter(
      (generation) => HARDWARE_GENERATION_PROFILES[generation].video.dynamicLight,
    );
