...
`package.json`（`import:textures` を足す）、`Docs/measurements/T1-21_material.md`
...
6. `T1-21_material.md` に 6 枚の名前を書き足す。**`check-textures.ts` は記録に
   全ファイル名が載っていることを検査する**（`check-textures.ts:429`）ので、忘れると落ちる。

**受け入れ**: `npx tsx tools/import-textures.ts && npm run make:textures && npm run check:assets`
が 68 枚すべてで通る。gen1 の 17 枚が宣言した番号へだけ落ちる。

**記録**: `Docs/measurements/SG-01_import.md`（変換規則・写像表・4 セットの色数と明度の実測）
...
**記録**: `Docs/measurements/SG-02_fc_palette.md`（総当たりの手続きと、7 色を選んだ理由）
...
**記録**: `Docs/measurements/SG-03_sky.md`（**差し替えたゴールデン 2 件の理由をここに残す**）
...
**記録**: `Docs/measurements/SG-04_top_face.md`
...
**記録**: `Docs/measurements/SG-05_decor.md`
...
**記録**: `Docs/measurements/SG-06_flora.md`（4 世代の画面と、置いた数）
...
**記録**: `Docs/measurements/SG-07_cloud.md`
...
**記録**: `Docs/measurements/SG-08_water.md`
...
**記録**: `Docs/measurements/SG-09_light.md`
...
**記録**: `Docs/measurements/SG-10_gate.md`

### SG-11 4 世代を並べる・予算・検査

**触るもの**: `Docs/measurements/SG-11_four_channels.md`、`tests/unit/`（三角形の見積り）
